<template>
	<view :class="['message', isMe ? 'me' : 'other']">
		...
		<view class="content-wrapper">
			...
			<!-- 红包消息卡片 -->
			<view :class="['redbag-card', message.status === 'claimed' ? 'claimed' : '', message.status === 'opened' ? 'opened' : '']" @click="handleClick">
				<view class="redbag-content">
					...
					<view class="redbag-info">
						...
						<!-- 未领取：显示当前累加金额 -->
						<view class="redbag-amount" v-if="message.status === 'unopened' && message.currentAmount > 0">
							<text class="amount-value">{{ message.currentAmount }}</text>
							<text class="amount-unit">金币</text>
						</view>
						<text v-else-if="message.status === 'unopened'" class="redbag-hint">点击拆红包</text>
						<!-- 已拆开 -->
						<text v-else-if="message.status === 'opened'" class="redbag-amount-final">
							{{ message.claimedAmount || message.amount || 0 }} 金币
						</text>
						<!-- 已领取 -->
