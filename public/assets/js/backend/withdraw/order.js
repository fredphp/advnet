                ...
                $.ajax({
                    ...
                    success: function (ret) {
                        if (ret.code == 1) {
                            var data = ret.data;
                            // 今日提现金额
                            $('#stat-today-total').text('¥' + (data.today_total || 0).toFixed(2));
                            $('#stat-today-count').text(data.today_count || 0);
                            // 待审核金额
                            $('#stat-pending-amount').text('¥' + (data.pending_amount || 0).toFixed(2));
                            $('#stat-pending-count').text(data.pending_count || 0);
                            // 待打款金额
                            $('#stat-approved-amount').text('¥' + (data.approved_amount || 0).toFixed(2));
                            $('#stat-approved-count').text(data.approved_count || 0);
                            // 已拒绝金额
                            $('#stat-rejected-amount').text('¥' + (data.rejected_amount || 0).toFixed(2));
                            $('#stat-rejected-count').text(data.rejected_count || 0);
                            // 已成功金额
                            $('#stat-success-amount').text('¥' + (data.success_amount || 0).toFixed(2));
                            $('#stat-success-count').text(data.success_count || 0);
                        }
                    }
                });
            },
